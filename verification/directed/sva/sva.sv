...
// Description
// -----------
// SystemVerilog Assertions (SVA) checker for the fibonacci DUT.
// Instantiated via bind in tb.sv inside the DUT scope, giving direct
// visibility of both ports and internal state registers (a, b).
//
// Assertion map
// ─────────────────────────────────────────────────────────────────────────────
...
//  AST_RST_A     | RST-001     | assert  | rst_ni low → a = 0 (async)
//  AST_RST_B     | RST-002     | assert  | rst_ni low → b = 1 (async)
//  AST_SEQ_A     | SEQ-001     | assert  | enable → a takes prev b (1-cycle)
//  AST_SEQ_B     | SEQ-002     | assert  | enable → b = prev_a + prev_b
//  AST_HOLD_OUT  | HLD-001     | assert  | !enable → fib_out_o stable
//  AST_HOLD_B    | HLD-001     | assert  | !enable → b stable
//  COV_EN_RISE   | —           | cover   | enable 0→1 transition observed
//  COV_EN_FALL   | —           | cover   | enable 1→0 transition observed
//  COV_RST_ASRT  | —           | cover   | rst_ni asserted (1→0)
//  COV_RST_REL   | —           | cover   | rst_ni released (0→1)
//  COV_EN_HOLD3  | HLD-001     | cover   | enable=0 for at least 3 cycles
//  COV_OVERFLOW  | OVF-001     | cover   | wrap-around: out[n] < out[n-1]
//                |             |         | while enable was high
// ─────────────────────────────────────────────────────────────────────────────
//
// Notes
// -----
...
//   the DUT's internal registers a and b (see bind statement in tb.sv).
//
// -----------------------------------------------------------------------------
...
module sva #(
  ...
  input logic [W-1:0]   fib_out_o,  // DUT port  — equals register a
  ...
  // AST_RST_A — RST-001
  // Internal register a must also be 0 during reset.
  ...
  // AST_RST_B — RST-002
  // Internal register b must be 1 during reset so the first enabled
  ...
  // AST_SEQ_A — SEQ-001
  // One cycle after enable is asserted, register a must equal the previous
  // value of b: implements the recurrence a(n) <= b(n-1).
  ...
  // AST_SEQ_B — SEQ-002
  // One cycle after enable is asserted, register b must equal $past(a)+$past(b),
  // verifying the Fibonacci recurrence F(n) = F(n-1) + F(n-2) at register level.
  ...
  // When enable is de-asserted, fib_out_o must not change on the next clock.
  ...
  // AST_HOLD_B — HLD-001
  // Internal register b must also remain stable while enable is low,
  ...
  // COV_EN_RISE — enable observed going 0 → 1
  cov_en_rise : cover property (
    $rose(enable_i)
  );

  // COV_EN_FALL — enable observed going 1 → 0
  ...
  // COV_EN_HOLD3 — enable held low for at least 3 consecutive cycles,
  ...
  // When enable is active and fib_out_o decreases (unsigned), the output has
  // wrapped around 2^W — exercising the OVF-001 requirement.
