...
// Reference model
// ---------------
// ref_a / ref_b mirror the DUT state registers using identical reset and
// advance rules. check_output() asserts DUT fib_out_o == ref_a every call.
//
// -----------------------------------------------------------------------------
...
module test #(
  ...
  // TC-RST-02 — Reset independent of enable
  //
  // Goal    : Reset asserted while enable=0; fib_out_o must still become 0.
  // Req     : RST-001
  // ---------------------------------------------------------------------------
  task automatic tc_rst_02();
    $display("\n[TEST] TC-RST-02 — Reset independent of enable");
    // Reach state: a=13, b=21 (F(6)/F(7) with default W).
    apply_reset();
  ...
  // TC-RST-03 — Correct sequence after reset release
  //
  // Goal    : After releasing reset with enable=1, sequence must be
  ...
  //
  // Goal    : With enable=1 continuously the DUT must match the golden model
  ...
  //
  // Goal    : Sequence advances only on cycles where enable=1; hold on the
  ...
  //
  // Goal    : After reaching fib_out_o=5, deassert enable for 10 cycles;
  ...
  // TC-HLD-03 — Reset during hold dominates
  //
  // Goal    : While enable=0 (hold), asserting rst_ni=0 must immediately
